...
def random_forest_classifier(train_x, train_y):
    ...
    model = RandomForestClassifier(n_estimators=350,random_state=1)
...
result={}
for featureindex in range(len(featuresdecription.pickfeature2)):
    pickedFeatures=featuresdecription.pickfeature2[0:1+featureindex]
    scores=[]
    for maxtime in range(1,49):
    # 1---48 hors
    #     if len(pickedFeatures) ==0:
    #         break
        if maxtime%6 !=0:
            continue
    ...
    # print('length: '+ str((len(pickedFeatures)))+"result: "+str(avg))
    # print(pickedFeatures)
    print(avg)
    result[featureindex]=avg
