    ...
    for file in files:
        if ('.txt'in file):#and title.replace(' ','_') in file):
            ...
            maplenthofTweet=maplenthofTweet.map(lambda v1:({'tweetid':v1[1]['tweet_id'],'features':{
                                                        ...
                                                        'UserDescription': getUserFromID(v1[1]["user_id"])['Description'] ,
            ...
            maplenthofTweetout={'eventID':mytweet['eventID'],'data':maplenthofTweet}

            with open(outputFile, mode='a') as writer:
                JSON=json.dumps(maplenthofTweetout)
                writer.write(JSON + '\n')
            with open(outputFilereadble, mode='a') as writer:
                writer.write(str(mytweet['eventID']) + '\n')
                for tweet in maplenthofTweetout['data']:
