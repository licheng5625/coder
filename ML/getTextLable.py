...
extendulrlist={}
with open(path.TweetJSONpath+'URLsRumors.txt',mode='r') as reader:
    for line in reader:
        urls=json.loads(line)
        extendulrlist[urls[0]]=urls[1]
with open(path.TweetJSONpath+'URLsNews.txt',mode='r') as reader:
    for line in reader:
        urls=json.loads(line)
        extendulrlist[urls[0]]=urls[1]

def getextendUrl(urllist,tweet):
    if len(urllist)==0:
        return []
    ret=[]
    for url in urllist:
        try:
            ret.append(extendulrlist[url])
        except KeyError:
            ret.append(tweet['extend_urls'])
    return ret
...
outputFile=path.Featurepath+'featuresNewsTextlable2.txt'
# datafolder=path.TweetJSONpath+'rumors/'
# descriptionFile=path.TweetJSONpath+'descriptionRumors.txt'
# outputFile=path.Featurepath+'featuresrumorsTextlable232.txt'
maplenthofTweetall={}
    ...
    for file in files:
        if ('.txt'in file):#and title.replace(' ','_') in file):
            ...
            def getAvg(Num,Toavg):
                ...
                return Volume2
            rootJSONMap=rootmap.map(lambda line2:(getTimefromJson(line2),json.loads(line2))).filter(lambda v1:v1[0]>=begindate).filter(lambda v1:v1[0]<=enddate).sortByKey(True,1)
            ...
            maplenthofTweet=rootJSONMap.map(lambda v1:({'tweetid':v1[1]['tweet_id'],'features':  v1[1]['text']}

                #{'tweetid':v1[1]['tweet_id'],'features':
            ...
            # try:
            #     hours.append(maplenthofTweet[0].days*24+maplenthofTweet[0].seconds/3600+1)
            #     print(maplenthofTweet[0].days*24+maplenthofTweet[0].seconds/3600+1)
            # except IndexError:
            #     print('no')
            # for keu in maplenthofTweet:
            #     try:
            #         maplenthofTweetall[keu[0]]+=maplenthofTweet[keu[1]]
            #     except KeyError:
            #          maplenthofTweetall[keu[0]]+=maplenthofTweet[keu[1]]
# with open(outputFile, mode='w') as writer:
#     JSON=json.dumps(maplenthofTweetall)
#     writer.write(JSON + '\n')
# print(sum(hours) / float(len(hours)))
# print(len(hours))