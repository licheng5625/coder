        ...
        if ('.txt'in file):
            ...
            with open(descriptionFile,encoding='utf-8',mode='r')as Seenlist2:
                for line in Seenlist2:
                    ...
                    print(datapath)
                    if file in data['tweetFile']:#== datapath:
                        mytweet=data
                        break
            ...
            rootJSONMap=rootJSONMap.filter(lambda v:len(v[1]['urls'])>0).flatMap(lambda line:line[1]['extend_urls']).map(lambda v:(v,1)).reduceByKey(lambda v1,v2:v1+v2).map(lambda v:v[0]).collect()#.map(lambda v:(unshortUrl.unshorten_url(v),1)).reduceByKey(lambda v1,v2:v1+v2).map(lambda v:v[0]).collect()
            #print(rootJSONMap)
#             for usr in rootJSONMap.collect():
#                 userlist[usr[1]]=usr[0]
            with open(outputFile, mode='a')as Seenlist2:
